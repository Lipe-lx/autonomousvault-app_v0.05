// services/ai/aiTypes.ts
// Unified types and interfaces for multi-provider AI support
...
export const PROVIDER_MODELS: Record<AIProviderType, ModelInfo[]> = {
    gemini: [
        ...
        { id: 'gemini-2.0-flash-lite', name: 'Gemini 2.0 Flash-Lite', description: 'Economy', inputPricePerMillion: 0.075, outputPricePerMillion: 0.30 },
    ],
    openai: [
...
export interface IAIProvider {
    ...
    // Batch analysis for Hyperliquid Dealer (returns decisions with optional cycleSummary)
    sendBatchAnalysis(
        context: any,
...
// Extended config for request queue (backwards compatible)
...
export const PROVIDER_RATE_LIMITS: Record<AIProviderType, RateLimiterConfig> = {
    // Gemini Free Tier: 15 RPM for stable models, 2-5 RPM for preview
    // Using 5s base interval (12 RPM safe margin) with dynamic adjustment
...
// Extended queue configs (used by requestQueue.ts)
