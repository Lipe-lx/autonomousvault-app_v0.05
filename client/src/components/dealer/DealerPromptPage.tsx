...
import { DealerState, INDICATOR_PRESETS, PRESET_PROMPTS, PresetName } from '../../state/dealerStore';


interface DealerPromptPageProps {
    status: DealerState;
    onSaveStrategy: (prompt: string) => void;
    onUpdateSettings?: (settings: Partial<DealerState['settings']>) => void;
}

export const DealerPromptPage: React.FC<DealerPromptPageProps> = ({
    status,
    onSaveStrategy,
    onUpdateSettings
}) => {
    // Debug logging
    useEffect(() => {
        console.log('DealerPromptPage Mounted. Status:', status);
    }, [status]);

    const settings = status?.settings;

    if (!settings) {
        return <div className="p-8 text-red-500">Error: Dealer settings not loaded.</div>;
    }

    const [strategyValue, setStrategyValue] = useState(settings.strategyPrompt || '');
    const [promptMode, setPromptMode] = useState<'preset' | 'custom'>(settings.promptMode || 'preset');
    const [selectedPreset, setSelectedPreset] = useState<PresetName>(settings.selectedPreset || 'balanced');
    const [strategyDirty, setStrategyDirty] = useState(false);

    // Sync with external settings changes
    useEffect(() => {
        setStrategyValue(settings.strategyPrompt);
    ...
    const handlePresetSelect = (preset: PresetName) => {
        if (preset === 'custom') {
            // Switching to custom - keep current text
            ...
            setStrategyValue(presetPrompt);
            setPromptMode('preset');
            setSelectedPreset(preset);
            setStrategyDirty(presetPrompt !== settings.strategyPrompt);
        }
    };
    ...
    const handleStrategyChange = (e: React.ChangeEvent<HTMLTextAreaElement>) => {
        ...
        // Check if the text matches any preset
        const matchingPreset = (Object.entries(PRESET_PROMPTS) as [PresetName, string][])
            .find(([_, prompt]) => prompt === newValue);

        if (matchingPreset) {
            setPromptMode('preset');
            setSelectedPreset(matchingPreset[0]);
        } else {
            // Edited -> switch to custom
            setPromptMode('custom');
            setSelectedPreset('custom');
        }

        setStrategyDirty(newValue !== settings.strategyPrompt);
    };

    const handleStrategySave = () => {
        onSaveStrategy(strategyValue);

        // Also update the prompt mode and preset in settings
        if (onUpdateSettings) {
            onUpdateSettings({
                promptMode,
                selectedPreset,
                strategyPrompt: strategyValue
            });
        }
    ...
    const getPresetButtonClass = (preset: PresetName) => {
                ...
                <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-4">
                        ...
                        `}>
                            ...
                            {promptMode === 'preset'
                                ? `${INDICATOR_PRESETS[selectedPreset]?.emoji} ${INDICATOR_PRESETS[selectedPreset]?.name}`
                                : 'Custom'
                            }
                        </div>
            ...
            <div className="flex-1 flex gap-4 min-h-0 overflow-hidden">
                ...
                <div className="flex-1 glass-panel relative overflow-hidden flex flex-col">
                    ...
                    <textarea
                        ...
                        spellCheck={false}
                        placeholder={`// Define your trading strategy here...
// 
// Example:
